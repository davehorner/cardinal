...
fn test_open_url_with_protocol_vars() {
    ...
    assert_eq!(result_enhanced.url, "https://wiki.xxiivv.com/etc/catclock.tal.txt");
    assert_eq!(result_enhanced.proto_vars.get("widget").unwrap().as_bool(), Some(true));
    assert_eq!(result_enhanced.proto_vars.get("debug").unwrap().as_bool(), Some(true));
    
    // Test with standard API (now enhanced) 
    let result_standard = ProtocolParser::parse(&url);
    assert_eq!(result_standard.url, "https://wiki.xxiivv.com/etc/catclock.tal.txt");
    assert_eq!(result_standard.proto_vars.get("widget").unwrap().as_bool(), Some(true));
    assert_eq!(result_standard.proto_vars.get("debug").unwrap().as_bool(), Some(true));
}