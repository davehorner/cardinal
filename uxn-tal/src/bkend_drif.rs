...
pub fn drifblim_repo_get_drifloon() -> PathBuf {
    ...
    if let Some(home) = home_dir {
        let drifloon_path = home.join(".uxntal").join(".drifblim").join("src").join("drifloon.rom");
...
pub fn drifblim_repo_get_drifblim() -> PathBuf {
    let home_dir = dirs::home_dir();
    if let Some(home) = home_dir {
        let drifblim_path = home.join(".uxntal").join(".drifblim").join("src").join("drifblim.rom");
...
pub fn drifblim_repo_get_drifblim_seed() -> PathBuf {
    let home_dir = dirs::home_dir();
    if let Some(home) = home_dir {
        let drifblim_seed_path = home.join(".uxntal").join(".drifblim").join("bin").join("drifblim-seed.rom");
...
pub fn ensure_drifblim_repo() -> Result<Option<PathBuf>, AssemblerError> {
    ...
    let home_dir = dirs::home_dir().ok_or_else(|| simple_err(Path::new("~/.uxntal/.drifblim"), "failed to get home directory"))?;
    ...
    if !drifblim_path.exists() {
        let status = Command::new("git")
            ...
            .arg(&drifblim_path)
            .status()
            .map_err(|e| simple_err(&drifblim_path, &format!("failed to execute git clone: {e}")))?;
        if !status.success() {
    ...
    if !drifblim_path.exists() {
        ...
        return Err(simple_err(&drifblim_path, "drifblim repository not found after clone/pull"));
    }
    let _guard = DirGuard::new(&drifblim_path);

    let drifblim_seed_path = uxntal_path.join(".drifblim").join("bin").join("drifblim-seed.rom");
    let drifblim_seed_txt_path = uxntal_path.join(".drifblim").join("etc").join("drifblim.rom.txt");
    HexRev::hex_to_bin_paths(&drifblim_seed_txt_path, &drifblim_seed_path).map_err(|e| simple_err(&drifblim_seed_txt_path, &format!("failed to convert hex to bin: {e}")))?;
    ...
    if !drifloon_rom.exists() {
        ...
        let ret = asm.assemble(&drifloon_tal_contents, Some(drifloon_rom.display().to_string()));
        match ret {
            Ok(rom) => {
                fs::write(&drifloon_rom, &rom)
                    .map_err(|e| simple_err(&drifloon_rom, &format!("failed to write rom: {e}"))).ok();
                if drifloon_rom.exists() {
                    eprintln!("Successfully assembled drifloon.tal {}", drifloon_rom.display());
                } else {
                    eprintln!("Assembly succeeded but drifloon.rom not found at {}", drifloon_rom.display());
    ...
    if !drifblim_rom.exists() {
        ...
        let ret = asm.assemble(&drifblim_tal_contents, Some(drifblim_rom.display().to_string()));
        match ret {
            Ok(rom) => {
                fs::write(&drifblim_rom, &rom)
                    .map_err(|e| simple_err(&drifblim_rom, &format!("failed to write rom: {e}"))).ok();
                if drifblim_rom.exists() {
                    eprintln!("Successfully assembled drifblim.tal {}", drifblim_rom.display());
                } else {
                    eprintln!("Assembly succeeded but drifblim.rom not found at {}", drifblim_rom.display());
